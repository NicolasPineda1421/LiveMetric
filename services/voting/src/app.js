...
// Historial del propio votante: se incluye "electionId" solo porque el
// frontend lo necesita para dejar de ofrecer la papeleta de una elección ya
// votada (evita el bug de volver a mostrar las opciones tras votar), pero
// nunca se hace JOIN con "elections" ni se selecciona "option_id" — el
// historial que se le muestra al votante es solo "votaste, a esta hora",
// nunca en qué eligió ni el nombre/pregunta de la elección.
app.get('/my-votes', requireVoter, async (req, res) => {
  const { voterIdHash } = req.voter;
  try {
    const result = await pool.query(
      `SELECT election_id, created_at FROM votes WHERE voter_id_hash = $1 ORDER BY created_at DESC`,
      [voterIdHash]
    );
    return res.status(200).json({
      votes: result.rows.map((r) => ({ electionId: r.election_id, createdAt: r.created_at })),
    });
  } catch (err) {
    console.error('Error en GET /my-votes:', err.message);
    return res.status(500).json({ error: 'Error interno del servidor' });
  }
});
...
// Lista TODAS las elecciones (cualquier estado). Lectura para admin y
// auditor (necesitan elegir una elección en Resultados/Reportes); el resto
// de rutas /admin/* de este servicio (crear, detener) siguen exigiendo admin.
app.get('/admin/elections', requireRole('admin', 'auditor'), adminLimiter, async (_req, res) => {
  try {
    const elections = await pool.query(
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      // Si la elección todavía es "scheduled" con scheduled_start en el
      // futuro (nunca llegó a abrir), un simple LEAST(scheduled_end, now())
      // dejaría scheduled_end ANTES de scheduled_start, violando el CHECK
      // valid_time_window (encontrado por las pruebas de integración: antes
      // esto respondía 500 en vez de cerrar la elección). Se cubren los dos
      // casos por separado: si nunca abrió, se le da una ventana mínima ya
      // vencida (1 segundo); si ya estaba en curso, se acorta como siempre.
      const result = await pool.query(
        `UPDATE elections
            SET status = 'closed',
                stopped_manually = true,
                scheduled_start = LEAST(scheduled_start, now()),
                scheduled_end = CASE
                  WHEN scheduled_start > now() THEN now() + interval '1 second'
                  ELSE LEAST(scheduled_end, now())
                END
          WHERE id = $1
